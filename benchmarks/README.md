...
- [`evidence/concurrency/catalog.json`](evidence/concurrency/catalog.json) keeps
  retained concurrency measurements in one human-readable catalog, grouped by
  stable subsystem and contract identifier.
...
- `probes/concurrency.zip` stores the matching standalone benchmark and TSan
  sources byte-for-byte in a deterministic archive.
...
Verify or stage the retained native probes for inspection and compilation:

```bash
python -m benchmarks.concurrency.assets verify
python -m benchmarks.concurrency.assets stage artifacts/concurrency-probes
```

After editing a complete staged tree, rebuild the deterministic archive with
`python -m benchmarks.concurrency.assets pack artifacts/concurrency-probes`.
...
commit and native-extension digest that were measured. A first budget breach
triggers two fresh measurements; CI fails only after three consecutive
breaches, preserving persistent-regression detection without treating one
hosted-runner pause as product behavior.
