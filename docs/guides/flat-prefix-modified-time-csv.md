...
event columns, or introduce new ones. The engine pre-reads each header, builds
immutable per-source projections, and emits nulls for missing fields. Output
order still follows `column_order`: the default is alphabetical, while
`schema_contract_first` puts registry fields first and appends newly discovered
fields deterministically. Duplicate headers, mixed header/no-header sources,
and rows wider than their own header remain errors.
...
inference, staging, and native materialization with `memory_limit_bytes`.
PyArrow, pandas, and Polars outputs become caller-owned outside the operation
ledger after handoff. A lazy DuckDB relation instead retains its governed
upstream chain until the final related proxy closes. A large daily dataframe
can therefore exhaust process memory even though the reader itself stayed
within budget. Example 8 intentionally uses `to_polars` because it performs a
custom vectorized wide-to-nested transformation; its day size must be chosen
with that dataframe risk in mind. See the
