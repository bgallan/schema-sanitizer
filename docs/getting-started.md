...
[Python API](python-api.md) and [options reference](options.md) cover the full
surface.
...
Analytical results are outside that budget once returned. Prefer a file output
or `iter_batches` when the complete table may not fit in memory.

## [Continue learning](#index)

- [Options](options.md) lists every conversion parameter.
- [Inputs and filesystems](inputs-and-filesystems.md) covers Python, local, and
  cloud sources.
- [Heuristics](heuristics.md) explains inferred types and schema evolution.
- [Pipelines](pipelines.md) covers recurring partitioned jobs.
- [Reader security limits](reader-security-limits.md) describes untrusted input
  handling.
