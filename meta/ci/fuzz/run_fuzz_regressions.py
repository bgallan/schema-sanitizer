...
def _stage_campaign_corpus(
    ...
    for source_root in source_roots:
        cases = regression_cases(source_root, target)
        if not cases:
...
def run_campaigns(
    ...
    *,
    ...
    Each campaign receives a temporary, byte-deduplicated input directory.
    When ``corpus_root`` is provided it is combined with the regressions;
    omitting it stages only the regression inputs used by older callers.
    """
    if runs <= 0:
        return 0

    with tempfile.TemporaryDirectory(prefix="schema-sanitizer-fuzz-") as temporary:
        campaign_root = Path(temporary)
        for target in TARGETS:
            _stage_campaign_corpus(
                campaign_root,
                regression_root,
                corpus_root,
                target,
