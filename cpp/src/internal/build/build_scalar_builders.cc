// Builds scalar Arrow C Data arrays from typed materialized cells.
...
public:
  ...
  Status finish(ArrowArray *out) override {
    ...
    payload->validity = std::move(validity_);
    if constexpr (std::is_same_v<T, double>) {
      payload->f64.assign(values_.begin(), values_.end());
    } else if constexpr (std::is_same_v<T, int64_t>) {
      payload->i64.assign(values_.begin(), values_.end());
    } else {
      payload->i32.assign(values_.begin(), values_.end());
    }
    payload->buffers.resize(2);
    payload->buffers[0] = validity_buffer(payload.get(), null_count_);
    if constexpr (std::is_same_v<T, double>) {
