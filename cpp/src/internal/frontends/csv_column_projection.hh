...
struct CsvProjectionStringHash {
  using is_transparent = void;

  // Hashes string-like keys without forcing temporary string allocation.
...
struct CsvProjectionStringEqual {
  using is_transparent = void;

  // Compares string-like keys without forcing temporary string allocation.
...
public:
  ...
  // Marks header processing complete when no header row was available.
  void finish_empty_header();
...
private:
  ...
  mutable std::unordered_map<std::string, const sanitize::FieldIndex *,
                             CsvProjectionStringHash, CsvProjectionStringEqual>
      root_field_cache_;
};
