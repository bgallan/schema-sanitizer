...
// Creates a frontend-specific direct materializer.
...
make_frontend_direct_materializer(PoolResource *pmr_pool, const char *name) {
  if (!pmr_pool)
    return sanitize::Status::Invalid(name,
                                     " direct materializer requires pmr_pool");
  try {
    return std::make_unique<DirectT>(pmr_pool);
  } catch (const std::bad_alloc &) {
    return sanitize::Status::OutOfMemory(name, " direct materializer: OOM");
  }
}
...
make_direct_materializer(std::string_view frontend_name,
  ...
  return sanitize::Status::Invalid("unsupported direct materializer frontend: ",
                                   frontend_name);
}
