...
[[nodiscard]] constexpr std::int64_t bounded_fraction(
    std::int64_t total, std::int64_t divisor, std::int64_t maximum,
...
memory_budget_from_limit(std::int64_t requested) noexcept {
  ...
  out.io_chunk_bytes = bounded_fraction(
      out.total_bytes, 64, 1LL * 1024LL * 1024LL);
  out.batch_target_bytes = bounded_fraction(
      out.total_bytes, 4, 64LL * 1024LL * 1024LL);
  out.coalesce_max_bytes =
      std::min<std::int64_t>(out.total_bytes, 512LL * 1024LL * 1024LL);
  out.metadata_bytes = bounded_fraction(
      out.total_bytes, 8, 256LL * 1024LL * 1024LL);
  ...
  out.parquet_reader_buffer_bytes = bounded_fraction(
      out.total_bytes, 2, 1LL * 1024LL * 1024LL * 1024LL);
  out.parquet_reader_rows = std::min<std::int64_t>(
      1'048'576, std::max<std::int64_t>(1,
                                        out.parquet_reader_buffer_bytes / 1024));
  out.parquet_row_group_bytes = bounded_fraction(
      out.total_bytes, 2, 512LL * 1024LL * 1024LL);
  out.parquet_row_group_rows = std::min<std::int64_t>(
      1'048'576,
      std::max<std::int64_t>(1, out.parquet_row_group_bytes / 1024));
  out.parquet_page_bytes = std::min<std::int64_t>(
      out.parquet_row_group_bytes,
      std::min<std::int64_t>(64LL * 1024LL * 1024LL,
  ...
  out.arrow_logical_slots = std::min<std::int64_t>(
      100'000'000,
      std::max<std::int64_t>(1'000'000, out.total_bytes / 8));
  out.arrow_logical_buffer_bytes =
      std::min<std::int64_t>(1LL * 1024LL * 1024LL * 1024LL,
                             out.total_bytes);
  out.async_concurrency = std::min<std::int64_t>(
      64, std::max<std::int64_t>(1, out.total_bytes / (16LL * 1024LL * 1024LL)));
  ...
  out.remote_chunk_prefetch = std::min<std::int64_t>(
      32, std::max<std::int64_t>(1, out.total_bytes / (32LL * 1024LL * 1024LL)));
  out.source_discovery_concurrency = std::min<std::int64_t>(
      128, std::max<std::int64_t>(1, out.total_bytes / (8LL * 1024LL * 1024LL)));
  return out;
}
