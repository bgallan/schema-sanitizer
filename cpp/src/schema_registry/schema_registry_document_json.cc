...
void append_registry_version(std::string &out, std::string_view output_name,
  ...
  internal::json_encoding::append_string_field(out, first, "output_name",
                                               output_name);
  internal::json_encoding::append_string_field(out, first, "schema", schema);
  internal::json_encoding::append_key(out, first,
...
void collect_registry_variants(std::vector<RegistryVariant> &records,
  ...
  for (const auto &field : fields) {
    const std::string_view source_segment =
        source_segment_for_output(field.name);
...
/// Returns the compatibility rank used to choose the current variant.
int compatibility_rank(TopLevelKind kind) {
  switch (kind) {
...
void append_registry_variants_json(std::string &out,
  ...
  for (std::size_t i = 0; i < records.size();) {
    ...
    if (!first_path)
      out.push_back(',');
    first_path = false;
    internal::json_encoding::append_string(out, source_path);
    out += ":{\"versions\":[";
...
std::string registry_json(const LogicalSchema &schema,
  ...
  out.push_back('{');
  bool first = true;
  internal::json_encoding::append_string_field(out, first, "field_name_policy",
                                               field_name_policy);
  internal::json_encoding::append_int_field(out, first, "registry_version", 1);
  internal::json_encoding::append_int_field(out, first, "schema_generation",
                                            generation);
  internal::json_encoding::append_key(out, first, "canonical_schema");
  append_canonical_schema_json(out, schema);
  internal::json_encoding::append_key(out, first, "variants");
  out.push_back('{');
  std::vector<RegistryVariant> records;
