// Parses strict integer and floating-point scalar text values.
...
// Appends a validated integer part with optional grouped thousands separators.
...
// Appends an optional decimal fraction.
...
// Appends an optional scientific exponent.
...
// Parses with floating std::from_chars only when the standard library exposes
// that overload. Supported Apple libc++ versions currently expose only the
// integral overloads.
...
// Portable fallback for libraries without floating std::from_chars.
...
bool parse_float64_strict(std::string_view s, char decimal_separator,
  ...
  if (!out)
    return false;
  if (s.empty() || decimal_separator == thousands_separator)
    return false;
