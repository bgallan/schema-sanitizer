// Options wire-format envelope and field codec.
//
// Heavy logical-schema section parsing lives in
// planning/io/options_schema_io.cc.
...
template <class Enum> static bool valid_enum_value(int32_t v) {
  if constexpr (std::is_same_v<Enum, SchemaEvolutionMode>) {
    return std::cmp_equal(v,
                          std::to_underlying(SchemaEvolutionMode::kStrict)) ||
           std::cmp_equal(v,
                          std::to_underlying(SchemaEvolutionMode::kAdditive));
  } else if constexpr (std::is_same_v<Enum, FieldOrderPolicy>) {
    return std::cmp_equal(
               v, std::to_underlying(FieldOrderPolicy::kAlphabetically)) ||
           std::cmp_equal(
               v, std::to_underlying(FieldOrderPolicy::kSchemaContractFirst));
  } else if constexpr (std::is_same_v<Enum, OnErrorPolicy>) {
    return std::cmp_equal(v, std::to_underlying(OnErrorPolicy::kStop)) ||
           std::cmp_equal(v, std::to_underlying(OnErrorPolicy::kSkipRow)) ||
           std::cmp_equal(v, std::to_underlying(OnErrorPolicy::kEmitNullRow));
  } else {
    return true;
  }
}
...
static sanitize::Status read_option_fields(std::string_view bytes,
                                           std::size_t *pos, Options *out) {
...
sanitize::Result<Options> deserialize_options(std::string_view bytes) {
  ...
  Options out;
  SAN_RETURN_NOT_OK(read_option_fields(bytes, &pos, &out));
