// Implements Python metadata-column parsing helpers for ABI3 streams.
...
/// Converts a Python Unicode object into an owned UTF-8 string.
...
/// Converts one supported Python object into a metadata column value.
bool py_value_to_metadata_column(PyObject *value, const char *placement_name,
  ...
  if (PyUnicode_Check(value)) {
    if (!py_unicode_to_string(value, "metadata columns", &column->value)) {
      return false;
    }
    return true;
  }
...
bool py_span_to_metadata_span(PyObject *obj, MetadataSpan *span) {
  ...
  if (static_cast<unsigned long long>(row_count) >
