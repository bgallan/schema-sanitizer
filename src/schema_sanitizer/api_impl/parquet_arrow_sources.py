"""Shared Parquet-to-native Arrow source planning helpers."""
...
from .parquet_errors import direct_parquet_memory_limit_error
...
class ParquetArrowSource:
    """One Parquet input that can be exposed as a native Arrow stream source."""
...
def _parquet_option_settings(call_options: Options | None) -> tuple[int | None, str]:
    """Return direct Parquet memory and timestamp settings from call options."""
...
def _set_route(set_route: RouteCallback | None, route: str) -> None:
    """Record a route decision when the caller exposes route diagnostics."""
    if set_route is not None:
        set_route(route)


def _close_factory(factory: Any) -> None:
    """Close a Parquet Arrow factory-like object."""
...
def close_parquet_arrow_sources(sources: Iterable[tuple[Any, str]]) -> None:
    """Close every factory in a native Arrow-source list."""
...
class ParquetArrowSourceChunkProvider:
    ...
    def next_sources(self) -> list[tuple[Any, str]] | None:
        ...
        if self._index >= len(self._sources):
            self.close()
            return None
        chunk = self._sources[self._index : self._index + self._chunk_size]
...
def parquet_arrow_stream_factory_or_none(
    ...
    *,
    ...
    if not parquet_schema_supports_direct_native_ingest(
        factory.schema,
        pa=pa,
