...
from ..api_impl.parquet_compression_options import pyarrow_parquet_writer_options
from ..core_impl.native_functions import COALESCING_STREAM_WRAP
...
from .pyarrow_parquet_common import DEFAULT_PARQUET_BATCH_ROWS
from .pyarrow_sink_lifecycle import open_pyarrow_output_sink

_LAST_PARQUET_COALESCE_ROUTE = "none"
_COALESCING_UNAVAILABLE_MESSAGES = (
    "requires an Arrow C stream",
    "requires the native C++ coalescing stream wrapper",
...
def _native_coalesced_reader(
    ...
    *,
    ...
    if wrap is None:
        raise RuntimeError(
            "Parquet output coalescing requires the native C++ coalescing stream wrapper."
        )
    capsule = wrap(batches, row_group_rows)
    if capsule is None:
        raise RuntimeError(
...
def write_parquet_stream(
    ...
    *,
    ...
    sink = open_pyarrow_output_sink(out_path, feature=feature)
    try:
        sink.writer = pq.ParquetWriter(
            sink.target,
            metadata.schema,
