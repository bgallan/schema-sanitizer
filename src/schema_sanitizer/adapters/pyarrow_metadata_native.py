...
from ..core_impl.native_functions import METADATA_STREAM_WRAP
...
def _metadata_stream_wrap_func() -> Any | None:
    """Return the cached native metadata stream wrapper when available."""
    return METADATA_STREAM_WRAP.get()
...
def native_metadata_reader(
    ...
    *,
    ...
    if not all(isinstance(name, str) for name in timestamp_columns):
        return None
    wrap = _metadata_stream_wrap_func()
    if wrap is None:
        return None
    capsule = wrap(
        stream,
