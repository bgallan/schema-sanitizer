...
def test_numeric_boundaries_match_single() -> None:
...
def test_escaped_strings_literals_and_empty_containers_match_single() -> None:
...
def test_nested_fallback_and_invalid_float_preserve_contract() -> None:
...
def test_wide_output_is_byte_identical(tmp_path: Path, require_native: None) -> None:
    ...
    single = ss.to_jsonl(source, single_path, multi_threading=False, **common)
    multi = ss.to_jsonl(source, multi_path, multi_threading=True, **common)
...
def test_flat_parser_avoids_hash_and_duplicate_primitive_scan() -> None:
