"""Regression coverage for concurrency sources bound column builders by packet slots."""
...
def _consume_strict(source: Path, output: Path, *, mode: str, contract: object):
    ...
    result = write_raw_stream_to_file(
        ...
        feature="sources-bound-column-builders-by-packet-slots stable column materializer slots",
...
def test_column_slots_are_reused_with_exact_single_multi_output(tmp_path: Path) -> None:
    ...
    if counters["column_groups_submitted"] > 0:
        ...
        # Row-parallel JSONL supersedes long-lived wide packets.
