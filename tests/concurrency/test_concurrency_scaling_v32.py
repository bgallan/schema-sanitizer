"""Regression coverage for v32 column-partitioned materialization."""
...
def _consume_strict_jsonl(
    ...
    return write_raw_stream_to_file(
        ...
        feature="v32 column partition regression",
...
def test_observed_nested_value_falls_back_without_reordering(tmp_path: Path) -> None:
    """A scalar contract with nested observed data returns to the v31 oracle."""
    require_native()
    contract = _strict_contract(tmp_path)
    rows = [
        {name: row_index + column for column, name in enumerate(_COLUMNS)}
...
def test_v32_sources_encode_bounded_reorder_and_exact_reparenting() -> None:
