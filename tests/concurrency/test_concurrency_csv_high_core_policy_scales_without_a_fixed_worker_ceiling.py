...
def test_csv_fixed_schema_packet_planning_is_constant_per_row() -> None:
...
def test_polars_disables_full_frame_rechunk_when_supported(
...
def test_polars_does_not_hide_conversion_type_errors(
    ...
    with pytest.raises(RuntimeError, match="could not convert.*Polars"):
        result_adapters.convert_arrow_stream_output(
            "stream",
            "polars",
            feature="csv-high-core-policy-scales-without-a",
...
def test_all_56_pairs_cover_csv_scaling_and_chunk_preserving_polars() -> None:
