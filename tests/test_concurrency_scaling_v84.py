"""Regression coverage for v84 transition-only arena publication."""
...
def test_v84_all_56_pairs_inherit_transition_only_publication() -> None:
    """Every supported source and sink crosses the common optimized arena."""
    ...
    for input_name, outputs in pairs.items():
        ...
        for output_name, guarantee in outputs.items():
            ...
            assert "empty_to_nonempty_queue_visibility" in shared
            assert "one_shot_worker_initialization_publication" in shared
            assert "park_boundary_first_task_sampling" in shared
            assert guarantee["source_to_sink_parallel_path"] is True
            assert guarantee["eligible_multi_benefit"] is True


def test_v84_native_mixed_lanes_preserve_exact_drain_and_worker_budget() -> None:
...
def test_v84_native_park_wake_cycles_keep_transition_masks_live() -> None:
