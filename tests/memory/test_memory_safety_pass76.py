        ...
        def __iter__(self):
            ...
            while True:
                ...
                if yielded and self.fail:
                    raise MemoryError("pass76 injected task-bank publication fault")
                yielded = True
                yield item
...
def test_terminal_ownership_underflow_quarantines_but_exact_slot_cleanup_survives() -> None:
    ...
    assert ledger.publish("pass76", 1, retained_bytes=64)
    ledger._owners = 0
    ledger.retire("pass76", 1)
    assert not any(slot.active for slot in ledger._slots)
    snapshot = ledger.snapshot()
    assert snapshot.owners == 0
    assert snapshot.rejected >= 1
    assert snapshot.corrupted is True
    assert ledger.publish("pass76", 2, retained_bytes=64) is False
...
def test_pass76_authoritative_paths_have_no_new_saturating_decrements() -> None:
    checks = {
