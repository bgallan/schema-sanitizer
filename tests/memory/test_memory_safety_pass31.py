"""Regression tests for pass31 real-terminal and ownership hardening."""
...
def test_cancelled_bridge_retains_submission_until_real_task_terminal() -> None:
    ...
    coordinator = RemoteIoCoordinator(
        ...
        permit_capacity=1,
        operation_id="pass31-real-terminal",
    )
...
def test_permit_delivery_failure_rolls_back_any_baseexception() -> None:
    ...
    try:
        ...
        waiter = module._Waiter(BrokenLoop(), future, 1, "pass31", "operation")
        with governor._lock:
...
def test_task_arena_plan_no_longer_owns_runtime_state() -> None:
    ...
    assert "benchmarks/pass31_arena_lifecycle_tsan.cc" not in source
    probe = (root / "benchmarks/pass31_arena_lifecycle_tsan.cc").read_text()
...
def test_remote_permit_restores_ownership_after_release_failure() -> None:
    ...
    governor = Governor()
    permit = RemoteIoPermit(governor, 1, "pass31")
    with pytest.raises(OSError, match="retry"):
        permit.release()
...
def test_callbackless_cancelled_before_start_releases_submission(
    ...
    owner._pid = os.getpid()
    owner._operation_id = "pass31-callbackless-cancelled"
