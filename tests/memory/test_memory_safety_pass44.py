"""Regression coverage for concurrency/memory hardening pass 44."""
...
def test_runtime_admission_cancels_reserved_activation() -> None:
    ...
    registration = registry.reserve(Service(), kind="pass44", close_name="close")
...
def test_runtime_service_requires_deadline_close_contract() -> None:
    ...
    with pytest.raises(TypeError, match="deadline_seconds"):
        _RuntimeServiceRegistry().reserve(Service(), kind="pass44", close_name="close")
...
def test_governor_preserves_physical_teardown_capacity() -> None:
    ...
    governor = _Governor(4, label="pass44", teardown_reserve=1)
...
def test_terminal_hosts_prune_dead_weakrefs_before_capacity_check() -> None:
    ...
    markers = TerminalHostMarkers(1, category="pass44_terminal")
    host = Host()
...
def test_operation_memory_lease_finalizer_only_transfers_owner(
    ...
    ledger = Ledger()
    lease = module.OperationMemoryLease(ledger, 1, "pass44")  # type: ignore[arg-type]
