"""Regression coverage for memory prepared finalizer legacy mismatch preserves capsule authority."""
...
def test_prepared_finalizer_legacy_mismatch_preserves_capsule_authority() -> None:
    ...
    with pytest.raises(RuntimeError, match="authority mismatch"):
        cancel_prepared_finalizer_cleanup(ticket + 1, capsule)
    assert capsule.ticket == ticket
    assert prepared_finalizer_capacity_snapshot()[1] == baseline + 1
...
def test_process_temporary_storage_capability_is_exact_and_non_replayable() -> None:
    ...
    first = governor.reserve_capability(
        1, path=path, label="prepared-finalizer-legacy-mismatch-preserves-capsule-first"
    )
    second = governor.reserve_capability(
        1, path=path, label="prepared-finalizer-legacy-mismatch-preserves-capsule-second"
    )
...
def test_remote_io_fault_before_commit_preserves_authoritative_owner(
    ...
    def fail_max(*_args: object, **_kwargs: object) -> int:
        raise MemoryError(
            "prepared-finalizer-legacy-mismatch-preserves-capsule injected precommit OOM"
        )

    monkeypatch.setattr(module, "max", fail_max, raising=False)
    with pytest.raises(MemoryError, match="prepared-finalizer-legacy-mismatch-preserves-capsule"):
...
def test_process_temporary_storage_capabilities_are_quarantined_across_fork_generation() -> None:
    ...
    capability = governor.reserve_capability(
        1,
        path=tempfile.gettempdir(),
        label="prepared-finalizer-legacy-mismatch-preserves-capsule-fork",
    )
...
def test_external_runtime_explicit_retirement_clears_known_stack_debt(
    ...
    result.owner.process_physical_thread_permits_release(result.amount)
...
def test_temporary_storage_borrow_state_oom_does_not_strand_capability_inflight(
    ...
    capability = governor.reserve_capability(
        1,
        path=tempfile.gettempdir(),
        label="prepared-finalizer-legacy-mismatch-preserves-capsule-borrow-oom",
    )
    original = governor._borrow_state

    def fail_borrow(*_args: object, **_kwargs: object) -> object:
        raise MemoryError("prepared-finalizer-legacy-mismatch-preserves-capsule borrow-state OOM")
