"""Regressions for concurrency/memory hardening pass 50."""
...
def test_required_memory_is_acquired_before_thread_fallback(
    ...
    admission = module.acquire_parallel_admission(
        2, per_slot_bytes=128, stage="pass50", require_memory=True
    )
    try:
...
def test_legacy_owner_activity_is_published_before_slot_visibility() -> None:
    ...
    body = source[start:end]
    # Pass85 makes the exact owner slot authoritative; derived activity is
