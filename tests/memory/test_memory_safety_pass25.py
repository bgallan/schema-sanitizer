"""Regression tests for pass25 retryable wrapper cleanup and fork-safe caches."""
...
def test_replay_file_path_survives_failed_unlink(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    replay._path = "/tmp/pass25-replay.arrow"
    calls = 0
    ...
    replay.close()
    assert replay._path == "/tmp/pass25-replay.arrow"
    replay.close()
...
def test_parquet_factory_retains_failed_pending_and_keepalive(
    ...
    factory = SimpleNamespace(
        ...
        _staged_path="/tmp/pass25.parquet",
    )
    removed = 0
    ...
    def remove(path: str | None) -> bool:
        ...
        nonlocal removed
        assert path == "/tmp/pass25.parquet"
        removed += 1
        return removed >= 2
    ...
    assert factory._staged_path == "/tmp/pass25.parquet"
