...
def test_fd_capability_counts_physical_open_and_close(tmp_path: Path) -> None:
    ...
    with module.acquire_file_descriptor_capability(1, label="pass62-test") as capability:
        with capability.open_descriptor(
            lambda: os.open(target, os.O_RDONLY), label="pass62-test-open"
        ) as descriptor:
            assert descriptor >= 0
...
def test_fd_capability_accounts_scandir_descriptor_duplication(tmp_path: Path) -> None:
    ...
    before = module._python_governed_fds_opened()
    with module.acquire_file_descriptor_capability(2, label="pass62-scandir") as capability:
        with capability.open_descriptor(
            lambda: os.open(tmp_path, flags), label="pass62-directory"
        ) as directory_fd:
            assert module._python_governed_fds_opened() == before + 1
            with capability.scandir(directory_fd, label="pass62-scandir") as entries:
...
def test_remote_local_file_adopts_preacquired_descriptor_credit(tmp_path: Path) -> None:
    ...
    try:
        with activate_remote_io_footprint(owner):
            with open_remote_local_file(path, "rb", label="pass62-remote-local") as handle:
