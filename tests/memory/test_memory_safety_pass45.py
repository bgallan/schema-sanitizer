"""Regression coverage for concurrency/memory hardening pass 45."""
...
def test_runtime_close_cannot_hide_concurrently_starting_thread() -> None:
    ...
    registration = registry.reserve(Service(), kind="pass45", close_name="close")
...
def test_operation_memory_release_uses_authoritative_per_lease_amount() -> None:
    ...
    ledger = FakeLedger()
    first = OperationMemoryLease(ledger, 10, "pass45")  # type: ignore[arg-type]
    second = OperationMemoryLease(ledger, 10, "pass45")  # type: ignore[arg-type]
...
def test_retry_rejects_hidden_owner_and_negative_charge(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    assert not scheduler.schedule(("pass45", 1), closure, delay_seconds=1.0, retained_bytes=256)
    with pytest.raises(ValueError, match="non-negative"):
        scheduler.schedule(("pass45", 2), lambda: None, delay_seconds=1.0, retained_bytes=-1)
...
def test_temporary_storage_finalizer_only_publishes_preallocated_owner(
    ...
    pool = module.TemporaryStoragePermitPool(None)
    lease = pool.try_acquire(1, label="pass45-finalizer", path=tmp_path)
