    ...
    (
        pytest.param(
            SourceContract(
                ...
                excludes=("_DIRECT_LEASE_SEQUENCE",),
            ),
            id="cross-process-direct-lease",
        ),
        pytest.param(
            SourceContract(
                "governed-headroom",
                "src/schema_sanitizer/core_impl/memory_budget.py",
                start="def process_memory_pressure_snapshot",
                end="\ndef ",
                contains=("exact.capacity_bytes - exact.reserved_bytes",),
            ),
            id="governed-headroom",
        ),
        pytest.param(
            SourceContract(
                "fork-manager-explicit-child-safety",
                "src/schema_sanitizer/core_impl/fork_manager.py",
                contains=(
                    "child_safe_without_prepare",
                    "if callback is None:",
                    'handler.mode == "child_safe" and handler.child_safe_without_prepare',
                    "contract_generation",
                ),
            ),
            id="fork-manager-explicit-child-safety",
        ),
    ),
)
def test_bounded_authority_source_contract(contract: SourceContract) -> None:
    """Simple source invariants share one named declarative assertion path."""
