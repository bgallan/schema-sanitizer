"""Regression coverage for concurrency/memory hardening pass 47."""
...
def test_provider_release_is_committed_despite_post_commit_index_oom() -> None:
    ...
    lease, delay = governor.try_acquire("pass47-endpoint")
    ...
    assert governor.snapshot("pass47-endpoint").in_flight == 0
...
def test_provider_circuit_expiration_storage_is_o_live_keys_not_failures() -> None:
    ...
    # Pass48 keeps exactly one mutable heap node per tracked key.  Repeated
...
def test_remote_permit_release_never_reactivates_after_scheduler_oom(
    ...
    async def scenario() -> None:
        ...
        permit = await governor.acquire(operation_id="pass47")
        monkeypatch.setattr(
            governor,
            "_grant_ready_locked",
...
def test_runtime_registry_constructor_oom_cannot_publish_invisible_service(
    ...
    with pytest.raises(MemoryError):
        registry.reserve(Service(), kind="pass47", close_name="close")
...
def test_runtime_thread_start_commit_does_not_report_diagnostic_failure(
    ...
    registration = registry.reserve(Service(), kind="pass47", close_name="close")
    exited = Event()
...
def test_level_triggered_availability_failure_leaves_autonomous_retry_debt(
    ...
    governor = module._Governor(1, "pass47", level_triggered_availability=True)
