"""Regressions for concurrency/memory hardening pass 51."""
...
def test_fork_manager_freezes_exact_generation_when_before_registers_handler(
    ...
    module.register_fork_handler("pass51-a", before=a_before, mode="prepared_swap")
    module._before()
    module._child()
...
def test_fork_manager_failed_prepare_never_authorizes_child(
    ...
    def fail_before() -> None:
        raise MemoryError("pass51")

    def child() -> None:
        events.append("child")

    module.register_fork_handler(
        "pass51-failed", before=fail_before, after_in_child=child, mode="prepared_swap"
    )
    module._before()
    module._child()
    assert events == []
...
def test_reserved_finalizer_ring_prepares_before_owner_visibility_and_recycle() -> None:
    ...
    # Pass85 retirement commits as owner-free RECYCLE_PENDING. Ring/counter
...
def test_control_plane_failed_prepare_does_not_consume_reusable_token(
    ...
    first = budget.reserve("pass51", 256)
    token = first.token
...
def test_retry_deadline_index_has_one_physical_node_per_logical_retry(
    ...
    for index in range(200):
        assert scheduler.schedule(("pass51", "same"), lambda: None, delay_seconds=3600 + index)
...
def test_low_core_estimates_retained_bytes_before_executor_mutex() -> None:
    ...
    # pass51 moved all retained-byte estimation to callers before store_outcome_locked.
...
def test_pass51_native_abi_surface_declares_direct_atomic_activity_writer() -> None:
