...
class _RetryingMemoryOwner:
    ...
    def reserve(self, amount: int, *, stage: str) -> None:
        ...
        assert stage in {"operation-memory-lease-release-remains-retryable", "directory_metadata"}
...
def test_operation_memory_lease_release_remains_retryable() -> None:
    ...
    lease = OperationMemoryLease(owner, 17, "operation-memory-lease-release-remains-retryable")
...
def test_operation_memory_lease_shrink_remains_retryable() -> None:
    ...
    owner = _RetryingMemoryOwner(failures=1)
    lease = OperationMemoryLease(owner, 20, "operation-memory-lease-release-remains-retryable")
...
def test_memory_lease_finalizer_skips_interpreter_shutdown(
    ...
    lease = module.OperationMemoryLease(
        owner, 9, "operation-memory-lease-release-remains-retryable"
    )
    monkeypatch.setattr(module, "runtime_is_finalizing", lambda: True)
    lease.__del__()
