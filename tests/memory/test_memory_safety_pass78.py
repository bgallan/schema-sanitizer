...
def test_process_resource_governor_repairs_from_exact_leases_and_quarantines() -> None:
    ...
    governor = _Governor(3, "pass78-governor")
...
def test_dynamic_control_plane_repairs_derived_mirrors_from_exact_owner_ledger() -> None:
    ...
    first = budget.reserve("pass78", 512)
    budget._reserved = 0
    budget._active = 0

    second = budget.reserve("pass78-second", 512)
...
def test_operation_memory_shrink_keeps_exact_physical_high_watermark() -> None:
    ...
    ledger._resize_python_lease(owner, 40, stage="pass78")
    assert entry.size_bytes == 40
    assert entry.physical_size_bytes == 100
    assert reserved == []

    ledger._resize_python_lease(owner, 120, stage="pass78")
    assert reserved == [(20, "pass78")]
...
def test_retry_scheduler_rebuilds_admission_from_exact_owner_mappings(
    ...
    assert scheduler.schedule(("pass78", "a"), lambda: None, delay_seconds=60, retained_bytes=512)
    scheduler._pending_bytes = 0
    scheduler._subsystem_bytes.clear()

    assert not scheduler.schedule(
        ("pass78", "b"), lambda: None, delay_seconds=60, retained_bytes=512
    )
