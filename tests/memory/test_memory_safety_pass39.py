"""Regression coverage for concurrency/memory hardening pass 39."""
...
def test_runtime_registry_closes_services_with_one_shared_deadline() -> None:
    ...
    service = Service()
    registration = registry.register(service, kind="pass39", close_name="close")
    closed, remaining = registry.close_all(
        deadline_ns=deadline_ns_from_timeout(1.0, name="pass39 registry")
    )
    assert closed == 1
    assert remaining == 0
