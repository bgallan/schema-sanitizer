...
def test_forwarding_modules_are_removed() -> None:
    ...
    assert not (
        Path(__file__).resolve().parents[1] / "src/schema_sanitizer/api_impl/registry_output"
    ).exists()
    ...
    ingest_owner = Path(__file__).resolve().parents[1] / "src/schema_sanitizer/api_impl/ingest.py"
