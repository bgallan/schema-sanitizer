"""Regression coverage for v73 ordered native Parquet output overlap."""
...
def test_v73_every_format_declares_an_eligible_multi_benefit_proof() -> None:
...
def test_v73_small_parquet_row_groups_use_the_common_output_arena() -> None:
...
def test_v73_page_indexes_are_encoded_only_after_absolute_commit() -> None:
...
def test_v73_parquet_output_parallelizes_and_remains_logically_exact(
    tmp_path: Path,
) -> None:
    ...
    if int(stats["multi"].get("effective_workers", 1)) < 4:
        pytest.skip("v73 row-group overlap requires at least four effective workers")
