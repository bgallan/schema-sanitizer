"""Regression coverage for v75 worker-authoritative JSON framing."""
...
def test_v75_complete_format_matrix_names_worker_structural_framing() -> None:
...
def test_v75_scanner_is_bounded_worker_authoritative_and_has_canonical_fallback() -> None:
...
def test_v75_object_arrays_keep_exact_single_multi_data_and_real_work(
...
def test_v75_deep_values_fall_back_without_changing_results(tmp_path: Path) -> None:
...
def test_v75_scalar_split_at_chunk_boundary_is_complete_and_parallel(tmp_path: Path) -> None:
...
def test_v75_malformed_arrays_keep_exact_single_multi_errors(tmp_path: Path, payload: str) -> None:
