...
def test_consolidated_ci_owns_modified_time_csv_validation() -> None:
    ...
    for phase in range(1, 9):
        assert f"tests/test_modified_time_csv_phase{phase}.py" in workflow
    assert "tests/test_modified_time_csv_phase7_integration.py" in workflow
    assert workflow.count("tests/test_modified_time_csv_phase5.py") >= 3
    assert "pytest -q -o pythonpath=." in workflow
    assert "check_downstream_install.py" in workflow
    assert "tests/test_modified_time_csv_phase5.py" in tsan_runner
